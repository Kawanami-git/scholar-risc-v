# scholar risc-v Pipeline with bypassing Code Documentation

This documentation describes the source code of the **scholar risc-v pipeline with bypassing** core.

The goal of this page is to provide a code-oriented entry point: where the RTL files are located, which module to read first, how the pipeline stages are connected, and where to look when debugging hazards, stalls, flushes, memory accesses, or writeback behavior.

For the global project overview, supported instructions, results, and learning context, start from the repository `README.md`.
...
This branch implements a simple in-order RISC-V pipeline with bypassing.
...
Each stage exchanges data with the next stage through dedicated SystemVerilog interfaces stored in `risc-v/common/`.
...
- data hazards resolution (if possible),
...
- detect and resolve data hazards from source and destination register information,
- request Decode stalls when data hazards cannot be resolved,
