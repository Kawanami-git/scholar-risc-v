...
\details
 Instruction Decode Unit (ID) for the scholar risc-vcore.
...
| 1.3     | 01/05/2026 | Kawanami   | Refactor signals name. Also add a fix for csr_ctrl. |
********************************************************************************
*/
...
#(
  ...
  *             immediate branch offset (BRANCH), or `csr_rdata_i`.
  *
    ...
    case (op)
      ...
      SYS_OP: begin
        if (funct3 == 0) begin
          ...
          op2 = (funct3 == 3'b101) ? '0 : csr_rdata_i;
        end
        else begin
          op1 = (funct3 == 3'b011) ? ~rs1_data_i : rs1_data_i;
          op2 = (funct3 == 3'b001) ? '0 : csr_rdata_i;
        end
