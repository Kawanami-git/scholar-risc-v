...
#(
  ...
  *
  * Backpressure:
  *  - If `ready` is low, MEM holds internal registers to ensure the memory
  *    transaction completes without losing the associated control.
  *
  * NOP injection:
  *  - If `ready` is high but `exe_valid_i` is low, MEM clears internal
  *    registers. This propagates a NOP-like uop downstream:
  ...
  */
  always_ff @(posedge clk_i) begin : exe2mem
    if (!rstn_i) begin
      ...
      // mem_op3_sel_q <= exe2mem_i.mem_op3_sel;
    end
    else if (ready) begin
      exe_out_q   <= '0;
      op3_q       <= '0;
  ...
  assign mem2ctrl_o.is_load   = |mem_ctrl_q[MEM_CTRL_WIDTH-2:0];
  /// Generate bypass for Decode
  assign mem2id_o.bypass      = csr_ctrl_q == CSR_IDLE ? exe_out_q : op3_q;
  ...
  assign op3                  = op3_q;
  // assign op3                  = mem_op3_sel_q == SEL_WB ? wb2mem_i.bypass : op3_q;
